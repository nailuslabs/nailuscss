...
import Commands from './usageProvider';
import Hovers from './getMatched';
import CodeFolding from './folding';
import Diagnostics from './annotation';
import Decorations from './selectionStyle';
...
export default class Extension {
  ...
  get<T>(key: string) {
    return getConfig<T>(`nailuscss.${key}`);
  }
  ...
  isAttr(word: string): boolean {
    ...
    return (
      (getConfig('nailuscss.enableAttrVariantCompletion') &&
        lastKey in this.variants) ||
      (getConfig('nailuscss.enableAttrUtilityCompletion') &&
  ...
  isAttrUtility(word?: string): string | undefined {
    ...
    const lastKey = word.match(/[^:-]+$/)?.[0] || word;
    return getConfig('nailuscss.enableAttrUtilityCompletion') &&
      lastKey in this.attrs
      ? lastKey
      : undefined;
  }
