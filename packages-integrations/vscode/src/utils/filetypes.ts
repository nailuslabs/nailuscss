...
const nailusPattern = String.raw`\Wnailus\`[^\`]*$`;
const htmlPattern = getConfig('nailuscss.enableEmmetCompletion') ? [ classPattern, applyPattern, emmetPattern, nailusPattern ] : [ classPattern, applyPattern, nailusPattern ];
...
if (getConfig('nailuscss.includeLanguages')) {
  ...
  //   }>
  // }
  const config = getConfig<{[key:string]: (string | { type?: string, patterns?: string[] })}>('nailuscss.includeLanguages');
  if (config) {
