...
export default class CodeFolding {
  ...
  async create(editor?: TextEditor) {
    ...
    for (const index of Array.from(Array(document.lineCount).keys())) {
      if (getConfig('nailuscss.foldByLength')) {
        this.decorations[index] = await decorateWithLength(index, document.lineAt(index).text, getConfig('nailuscss.foldLength'), getConfig('nailuscss.hiddenTextColor'), getConfig('nailuscss.hiddenText'));
      } else {
        this.decorations[index] = await decorateWithCount(index, document.lineAt(index).text, getConfig('nailuscss.foldCount'), getConfig('nailuscss.hiddenTextColor'), getConfig('nailuscss.hiddenText'));
      }
    }
