...
import { rem2px } from './contextLoader';
...
export function buildStyle(styleSheet?: StyleSheet) {
  return styleSheet ? highlightCSS(getConfig('nailuscss.enableRemToPxPreview') ? rem2px(styleSheet.build(), getConfig('nailuscss.rootFontSize')) : styleSheet.build()) : undefined;
}
