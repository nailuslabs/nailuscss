...
export default class Diagnostics {
  ...
  constructor(processor?: Processor) {
    ...
    this.collection = languages.createDiagnosticCollection('nailus');
  }
      ...
      if (lineOfText.text.includes(seperator)) {
        ...
        if (match && match.index && this.processor) {
          ...
          for (const utility of this.processor.validate(utilities).ignored) {
            ...
            const diagnostic = new Diagnostic(range, `${utility.className} is not valid nailus css class`, DiagnosticSeverity.Error);
            diagnostic.code = 'nailus_invalid-class';
