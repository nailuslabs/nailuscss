  ...
  register(ext: DocumentSelector) {
    return languages.registerHoverProvider(ext, {
      provideHover: (document, position) => {
        ...
        const style = this.extension.isAttr(key) ? this.processor.attributify({ [key]: [ word ] }) : ['className', 'class', 'nailus', 'nailus'].includes(key) || text.match(applyRegex) ? this.processor.interpret(word) : undefined;
